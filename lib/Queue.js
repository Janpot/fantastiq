...
function createClient(redisOpts) {
  if (redisOpts && redisOpts.connection_option) {
    // assume this is a instancof RedisClient
    return redis.createClient(redisOpts.connection_option);
  } else {
...
Queue.prototype.quit = function () {
  return Promise.map(this._maintenanceCycles, function (cycle) {
    return cycle.stop();
  })
    .bind(this)
    .then(function () {
...
Queue.prototype._startMaintenanceCycles = function (time) {
  this._maintenanceCycles.push(
    this._startMaintenanceCycle(this._runTimeoutCycle.bind(this, time), time)
  );
  this._maintenanceCycles.push(
    this._startMaintenanceCycle(this._runCleanupCycle.bind(this, time), time)
  );
  this._maintenanceCycles.push(
    this._startMaintenanceCycle(this._runDelayedCycle.bind(this, time), time)
  );
  this._maintenanceCycles.push(
    this._startMaintenanceCycle(this._runMetricsCycle.bind(this, time), time)
  );
};
