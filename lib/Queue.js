...
Queue.prototype._getCleanupTime = function (state) {
  var configKey = {
  ...
  return this.config().get(configKey).then(function (period) {
    return Date.now() - period;
  });
};

Queue.prototype._cleanupState = function (lock, state) {
  return this._getCleanupTime(state)
    .then(function (cleanupTime) {
      return this._cleanupOlderThan(lock, state, cleanupTime, 0);
    });
};
