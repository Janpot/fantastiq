...
Queue.prototype.addN = function (jobs, options) {
  ...
  return this._scripts.run(SCRIPT_ADD_JOB, [
    ...
    this._key('job-data'),
    this._key('job-created')
  ], [ Date.now(), priority ].concat(jobs.map(this._stringifyData, this)));
};
...
Queue.prototype.retrieve = function (options) {
  return this.retrieveN(1, options)
    .bind(this)
    .then(function (result) {
      result.id = result.ids[0] || null;
      delete result.ids;
      return result;
    });
};

Queue.prototype.retrieveN = function (count, options) {
  options = options || {};
...
Queue.prototype.stat = function () {
  ...
  return this._client.exec()
    .then(function (results) {
      return {
        totalCount: results[0] + results[1] + results[2] + results[3],
