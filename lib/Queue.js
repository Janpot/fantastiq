...
Queue.prototype.getN = function (jobIds) {
  ...
  return this._scripts.run('getJob.lua', [
    this._key('job-state'),
      ...
      return jobIds.map(function (jobId, index) {
        ...
        if (state) {
          return {
            ...
            attempts: this._parseData(results[9][index])
          };
        } else {
          return null;
        }
...
Queue.prototype.addN = function (jobs, options) {
  ...
  return this._scripts.run('addJob.lua', [
    this._key('next-id'),
    this._key('inactive'),
    this._key('delayed'),
    this._key('job-state'),
...
Queue.prototype.removeN = function (jobIds) {
  ...
  return this._scripts.run('removeJob.lua', [
    this._key('inactive'),
    this._key('active'),
    this._key('completed'),
    this._key('failed'),
...
Queue.prototype.retrieve = function (options) {
  options = options || {};
  return this._scripts.run('retrieveJob.lua', [
    this._key('inactive'),
    this._key('active'),
    this._key('job-state'),
    this._key('job-started'),
...
Queue.prototype.acknowledge = function (jobId, error, result) {
  ...
  return this._scripts.run('acknowledgeJob.lua', [
    this._key('inactive'),
    this._key('active'),
    this._key('failed'),
    this._key('completed'),
...
Queue.prototype._runTimeoutCycle = function (lockTime) {
  return Promise.resolve(lockTime)
    ...
    .then(function () {
      return this._scripts.run('timeoutCycle.lua', [
        this._key('inactive'),
        this._key('active'),
        this._key('failed'),
        this._key('job-started'),
...
Queue.prototype._runCleanupCycle = function (lockTime) {
  return Promise.resolve(lockTime)
    ...
    .then(function (lock) {
      return this._scripts.run('cleanupCycle.lua', [
        this._key('job-marked-for-del'),
        this._key('failed'),
        this._key('completed'),
        this._key('tmp'),
...
Queue.prototype._runDelayedCycle = function (lockTime) {
  return Promise.resolve(lockTime)
    ...
    .then(function () {
      return this._scripts.run('delayedCycle.lua', [
        this._key('delayed'),
        this._key('inactive'),
        this._key('job-priority'),
        this._key('job-state'),
...
Queue.prototype.range = function (state, options) {
  ...
  return this._scripts.run('rangeJobs.lua', [
    this._key(state)
